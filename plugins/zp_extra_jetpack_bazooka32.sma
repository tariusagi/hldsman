...
// UZI_BEGIN: adjust jetpack cost.
#define JETPACK_COST 15 // set how may ammopacks the Jatpack+Rocket cost
// UZI_END.
...
public plugin_init() {
	...
	// UZI_BEGIN: register say command to buy a jetpack.
	register_clcmd("say /jp","cmd_buy_jetpack", ADMIN_ALL,": Buy a jetpack and bazooka");
	// UZI_END.
...
// UZI_BEGIN: buy the calling user (must have ADMIN_IMMUNITY) a jetpack.
public cmd_buy_jetpack(id)
{
	if (access(id, ADMIN_IMMUNITY))
	{
		new clip, ammo;
		new weapon = get_user_weapon(id, clip, ammo);
		client_print(id, print_chat, "[ZP] say /jphelp for Display the help page");
		hasjet[id] = 1;
